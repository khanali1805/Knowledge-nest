...
export const metadata: Metadata = {
  ...
  description: "Browse featured Knowledge Nest articles from every category.",
};
export default async function FeaturedPage() {
  const publishedArticles = await getPublishedArticles(100);
  const featuredArticles = publishedArticles.filter((article) => article.isFeatured);
  return (
    <>
      <SiteHeader />
      <main className="mx-auto min-h-[60vh] max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <p className="mt-4 max-w-3xl leading-7 text-slate-600">
          Featured stories selected from all active categories.
        </p>
